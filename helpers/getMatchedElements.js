    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (!foundURLObj.url) {
                ...
                if (!url)
                    reject({ error: true, message: "Result couldn't be fetched.Please try again later!" });
                fileUrl = url[0][0];
            ...
            try {
                ...
                if (e.code === "ECONNREFUSED") {
                    reject({ error: true, message: "Connection Refused From Moss Server.Please try again later!" });
                }
            }
            ...
            if ($("table tbody tr:nth-of-type(2) td:first-of-type a")[0]) {
                ...
                resolve({ textArr, matchedPercentage, matchedObjects, match: true, comment, sources: foundURLObj.sources, error: false });
            }
            reject({ match: false, comment, sources: foundURLObj.sources, error: false });
